...
{
    Matrix temp = mat;
    mc.LargerThan(temp.dev_data, temp.row, temp.col, num);
    temp.DataTransfer(DeviceToHost);
    return temp;
}
...
__global__ void TBR(MYTYPE* res_mat, MYTYPE* thre_mat, const int row, const int col)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    if (i < row && j < col)
    {
        if (thre_mat[i * col + j] != 0.0)
            res_mat[i * col + j] = 0.0;
    }
}
void Matrix::ThresholdBasedReset(const Matrix& mat)
{
    assert(row == mat.row && col == mat.col);
    dim3 threads(16, 16, 1);
    dim3 blocks((row + threads.x - 1) / threads.x, (col + threads.y - 1) / threads.y, 1);
    TBR <<<blocks, threads>>> (dev_data, mat.dev_data, row, col);
    cudaDeviceSynchronize();
    cudaMemcpy(data, dev_data, sizeof(MYTYPE) * element_num, cudaMemcpyDeviceToHost);
}

__global__ void TBRN(MYTYPE* res_mat, MYTYPE num, const int row, const int col)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    if (i < row && j < col)
    {
        if (res_mat[i * col + j] > num || res_mat[i * col + j] < -num)
            res_mat[i * col + j] = 0.0;
    }
}
void Matrix::ThresholdBasedReset(const MYTYPE num)
{
    dim3 threads(16, 16, 1);
    dim3 blocks((row + threads.x - 1) / threads.x, (col + threads.y - 1) / threads.y, 1);
    TBRN <<<blocks, threads >>> (dev_data, num, row, col);
    cudaDeviceSynchronize();
    cudaMemcpy(data, dev_data, sizeof(MYTYPE) * element_num, cudaMemcpyDeviceToHost);
}

__global__ void BYT(MYTYPE* mat, const int row, const int col, const MYTYPE num)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    if (i < row && j < col)
    {
        mat[i * col + j] = (mat[i * col + j] > num) || (mat[i * col + j] < -num);
    }
}
Matrix Matrix::BeyondThreshold(const MYTYPE num)
{
    Matrix temp = (*this);
    dim3 threads(16, 16, 1);
    dim3 blocks((row + threads.x - 1) / threads.x, (col + threads.y - 1) / threads.y, 1);
    BYT <<<blocks,threads>>> (temp.dev_data, row, col, num);
    cudaDeviceSynchronize();
    cudaMemcpy(temp.data, temp.dev_data, sizeof(MYTYPE) * temp.element_num, cudaMemcpyDeviceToHost);
    return temp;
}
